...
(function ($) {
    ...
    function refreshSelected() {
        let container = $("#selected-plans div");
        container.empty();

        $("#plan-badges input:checked").each(function () {
            let parent = $(this).closest("label");
            let clone = parent.clone(true);

            clone.removeClass("btn-outline-primary d-none")
        ...
        $("#selected-plans").toggleClass("d-none", container.children().length === 0);
    }

    $(document).on("click", "#plan-badges label", function () {
        let checkbox = $(this).find("input[type=radio]");
        if (!checkbox.prop("checked")) {
            checkbox.prop("checked", true);
            refreshSelected();
        }
        // إذا كان مختار من قبل، ما نفعل شيء
    });

    $(document).on("click", "#selected-plans .selected-item", function () {
        let id = $(this).data("id");
        let original = $('#plan-badges label[data-id="' + id + '"]');
